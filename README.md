...
`@gravity-ui/app-builder` is ESM-only and requires Node.js 20.19 or newer. Use `import`
or dynamic `import()` to load its API; CommonJS `require()` is not supported.
...
- app-builder.config.mts
- app-builder.config.js
- app-builder.config.mjs
- app-builder.config.cjs
...
- `cdn` (`CdnUploadConfig | CdnUploadConfig[]`) - upload bundled client files to CDN.
  ...
  - `hosts` (`string | RegExp | Array<string | RegExp>`) — hosts this CDN may be used on by the `publicPathFallback` runtime. A string is matched against the whole `location.hostname`, a `RegExp` is tested against it; matching is always case-insensitive. When omitted, the CDN is used on every host. Does not affect the upload — files are still uploaded to every configured bucket.
- `publicPathFallback` (`boolean | object`) — retry loading async chunks from another public path when the current one fails. Disabled by default.

  When a chunk fails to load, the runtime switches to the next candidate public path and sticks to it for all subsequent loads, so only the first failing chunk pays for a degraded CDN. Failures are reported with `console.warn`.

  The candidate list is fully resolved at build time from `cdn[*].publicPath` in declaration order, with `publicPath` appended last. Entries carrying `cdn[*].hosts` are only used when `location.hostname` matches, which is how a region gets its own backup. `window.__PUBLIC_PATH__` is always tried first, even if it is not in the list.

  - `includeLocalPublicPath` (`boolean=true`) — append `publicPath` (`/build/` by default) as the last candidate
  ...
  export default defineConfig({
    client: {
      cdn: [
        {bucket: 'assets', publicPath: 'https://cdn.example.com/build/'},
        {bucket: 'assets-ru', publicPath: 'https://cdn.example.ru/build/', hosts: /\.ru$/},
        {bucket: 'assets-kz', publicPath: 'https://cdn.example.kz/build/', hosts: 'app.example.kz'},
      ],
      publicPathFallback: true,
    },
  });
  // on app.example.ru: https://cdn.example.com/build/ -> https://cdn.example.ru/build/ -> /build/
  // on app.example.kz: https://cdn.example.com/build/ -> https://cdn.example.kz/build/ -> /build/
  // anywhere else:     https://cdn.example.com/build/ -> /build/
  ```

  Only lazy JS chunks and async CSS chunks are covered. Notably **not** covered: the initial `<script>` and `<link>` tags in the page HTML (they are requested before any bundle code runs), image/font/`new URL()` asset URLs (built when a module is evaluated, although modules evaluated after a switch do use the new path), `prefetch`/`preload` hints, and worker scripts.

  The option has no effect in dev mode, with `moduleFederation` (remotes load their entries and chunks through their own runtime, which the fallback cannot reach — a warning is logged), or when fewer than two distinct candidates are resolved. Note that `--cdn false` also disables it, since it removes the `cdn` config the list is derived from.
