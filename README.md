...
- `publicPathFallback` (`Array<string | {publicPath: string, hosts?: string | RegExp | Array<string | RegExp>}>`) — public paths to retry an async chunk from when the current one fails, in order. Disabled by default.

  When a chunk fails to load, the runtime switches to the next candidate and sticks to it for all subsequent loads, so only the first failing chunk pays for a degraded CDN. Failures are reported with `console.warn`.

  `window.__PUBLIC_PATH__` is always tried first, so this list usually holds only the backups — there is no need to repeat the primary public path here, and a candidate equal to it is deduplicated. This also means a single entry is already useful. Entries carrying `hosts` are used only when `location.hostname` matches, which is how a region gets its own backup: a string is matched against the whole hostname, a `RegExp` is tested against it, and matching is always case-insensitive.
  ...
  The list is independent of `cdn`, which only uploads. Use it whether the upload is done by app-builder or by something else entirely.

  If you add the local public path as the last candidate, write the value your build actually serves from: it is `/build/` by default, but `publicPathPrefix` and module federation asset isolation both change it.

  Only lazy JS chunks and async CSS chunks are covered. Notably **not** covered: the initial `<script>` and `<link>` tags in the page HTML (they are requested before any bundle code runs, so a primary outage there stops the app from booting and has to be handled in the page template or at the edge), image/font/`new URL()` asset URLs (built when a module is evaluated, although modules evaluated after a switch do use the new path), `prefetch`/`preload` hints, and worker scripts.

  The option has no effect in dev mode, with `moduleFederation` (remotes load their entries and chunks through their own runtime, which the fallback cannot reach — a warning is logged), or when fewer than two distinct candidates are resolved at runtime.
